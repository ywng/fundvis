 ...
 * Originally written by Chris Barr @ https://github.com/chrisbarr/AmazonSNS-PHP-API
 * Rewritten by Ben Chan
 *
 * @author Chris Barr, Ben Chan
...
setEndpointAttributes
	...
	publish (but to targetArn only)
	listPlatformApplications
	deleteEndpoint
	*/

	public function createPlatformEndpoint($customUserData, $platformApplicationArn, $token) {
		// customUserData is the name just to identify the device
		if (empty($platformApplicationArn) || empty($token)) {
			throw new InvalidArgumentException('Must supply a valid platformApplicationARN and token to create an endpoint');
		}

		$params = array();

		if (!empty($customUserData)) {
			$params['CustomUserData'] = $customUserData;
		}
		$params['PlatformApplicationArn'] = $platformApplicationArn;
		$params['Token'] = $token;
		$resultXml = $this->_request('CreatePlatformEndpoint', $params);

		return strval($resultXml->CreatePlatformEndpointResult->EndpointArn);
	}


	public function publishToEndpoint($messageJSON, $targetArn) {
		if (empty($messageJSON) || empty($targetArn)) {
			throw new InvalidArgumentException('Must supply a valid message JSON and targetARN to publish');
		}

		$resultXml = $this->_request('Publish', array('Message' => $messageJSON, 'MessageStructure' => 'json', 'TargetArn' => $targetArn));

		return strval($resultXml->PublishResult->MessageId);
	}

	public function listPlatformApplications($nextToken = null) {

		$params = array();
		if (!is_null($nextToken)) {
			$params['NextToken'] = $nextToken;
		}
		$resultXml = $this->_request('ListPlatformApplications', $params);

		$platformApplicationsArn = array();

		foreach ($resultXml->ListPlatformApplicationsResult as $key => $value) {
			$platformApplicationsArn[] = strval($value->Member->PlatformApplicationArn);
		}

		return $platformApplicationsArn;
	}

	public function deleteEndpoint($endpointArn) {
		if (empty($endpointArn)) {
			throw new InvalidArgumentException('Must supply a valid EndpointARN to delete an endpoint');
		}

		$this->_request('DeleteEndPoint', array('EndpointArn' => $endpointArn));
